  ...
  useEffect(() => {
    ...
    if (logoImgRef.current) {
      ...
      introTimeline
        .fromTo(
          logoImgRef.current,
          { rotation: 0, scale: 1, opacity: 0 },
          { rotation: 360, scale: 1.2, opacity: 1, duration: 0.75, ease: 'power2.out' }
        )
        .to(logoImgRef.current, { scale: 1, duration: 0.75, ease: 'power2.inOut' })
    }

    // ロゴテキストのアニメーション
    gsap.fromTo(logoRef.current?.querySelectorAll('.logo-name, .logo-subtitle'),
      {
        opacity: 0,
        x: -20,
      },
  ...
  return (
    <header ref={headerRef} className="header">
      <div ref={logoRef} className="logo"
        ...
        <img
          ref={logoImgRef}
          src="/OH-CAMERA-LOGO.png"
          alt="OH-CAMERA Logo"
          className="logo-img"
        />
        <div className="logo-text-container">
          <span className="logo-name">OH-CAMERA</span>
